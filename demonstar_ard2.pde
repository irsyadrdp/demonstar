...
//import
import gifAnimation.*;                //GIF animation
import processing.serial.*;           //Arduino
Serial myPort;                        //Arduino
  ...
  if(gameState==0){        //opening
    ...
    //background
        ...
        //background movement
          ...
          bg_height +=1;
          if (bg_height == 0){ 
            bg_height=-2400; background.bgPos.set(0, -2400, 0); 
          }
...
void shoot() {
  ...
  //handle fire button on keyboard
    final int bulletDens = 10;     //bullet density
    if((buttons[17] || buttons[32]) && frameCount % bulletDens == 0){
      plane.addBullet();
    }  
  
    //button for starting the game
    if(gameState==0 || gameState==2){
      if(buttons[17] || buttons[32]){ 
        gameState=1;     //launch to gameState1 to play the game
        livesAmount=3;   //default amount of lives for the beginnning of the game
        plane.score=0;   //reset score to 0
      }
    }   
}
...
void arduinoMovement(){
  ...
  if(myPort.available()>0){
    ...
    int getPort = myPort.read();
    print(val);
    
    //start game
    if(gameState==0  || gameState==2){  //opening and game over display state
      if(getPort==4) {
        gameState=1;
        livesAmount=3;
        plane.score=0;
    ...
    if(gameState==1){   //main gameplay state 
      //arduinno button pressed
      if(getPort==4 && frameCount % bulletDens == 0 ) plane.addBullet();  //shoot
      if(getPort==3) plane.planeSpeed.x = -plane.planeVel;                //right      
      if(getPort==2) plane.planeSpeed.x = plane.planeVel;                 //left
...
void addEnemy(int eAmount){
  if(frameCount % eAmount == 0){
    int enemeSize=80;
    enemies.add(new Enemy(random(width-enemeSize), -50, 
                new PVector(0,3), enemeSize, (int)random(1,6)));
  }
}
